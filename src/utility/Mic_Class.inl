...
#error "Mic_Class.inl is part of M5Unified.cpp and is not meant to be included on its own"
#endif
...
namespace m5
{
 ...
 #define MIC_CLASS_ADC_ATTEN_DB ADC_ATTEN_11db
  ...
  static i2s_chan_handle_t _mic_i2s_handle[M5UNIFIED_I2S_PORT_COUNT] = { nullptr, };

  static esp_err_t _mic_i2s_start(i2s_port_t port) {
    if (_mic_i2s_handle[port] == nullptr) { return ESP_FAIL; }
    return i2s_channel_enable(_mic_i2s_handle[port]);
  }
  static esp_err_t _mic_i2s_stop(i2s_port_t port)
  {
    if (_mic_i2s_handle[port] == nullptr) { return ESP_OK; }
    return i2s_channel_disable(_mic_i2s_handle[port]);
  }
  static esp_err_t _mic_i2s_read(i2s_port_t port, void* buf, size_t len, size_t* result, uint32_t timeout_ms) {
    if (_mic_i2s_handle[port] == nullptr) { return ESP_FAIL; }
    /// i2s_channel_read のタイムアウトはミリ秒 (旧 i2s_read の tick 数とは契約が異なる)
    return i2s_channel_read(_mic_i2s_handle[port], buf, len, result, timeout_ms);
  }
  static esp_err_t _mic_i2s_driver_uninstall(i2s_port_t port)
  {
    if (_mic_i2s_handle[port] != nullptr) {
      auto res = i2s_del_channel(_mic_i2s_handle[port]);
      /// Keep the handle when the deletion fails. (e.g. the channel is still running)
      if (res == ESP_OK) { _mic_i2s_handle[port] = nullptr; }
      return res;
    }
    return ESP_OK;
  }
  ...
  static esp_err_t _mic_i2s_set_adc(i2s_port_t port, gpio_num_t pin_data_in) {
    if (port == I2S_NUM_0)
  ...
  esp_err_t Mic_Class::_setup_i2s(void)
  {
    ...
    esp_err_t err = _mic_i2s_driver_uninstall(_cfg.i2s_port);
    if (err != ESP_OK) { return err; }
    err = i2s_new_channel(&chan_cfg, nullptr, &_mic_i2s_handle[_cfg.i2s_port]);
    if (err != ESP_OK) { return err; }
...
if (_cfg.pin_bck < 0 || _cfg.pin_ws < 0) {
    ...
    err = i2s_channel_init_pdm_rx_mode(_mic_i2s_handle[_cfg.i2s_port], &i2s_config);
} else
#endif
{
    ...
    i2s_config.gpio_cfg.din  = (gpio_num_t)_cfg.pin_data_in;
    err = i2s_channel_init_std_mode(_mic_i2s_handle[_cfg.i2s_port], &i2s_config);
}
    if (err != ESP_OK)
    {
      _mic_i2s_driver_uninstall(_cfg.i2s_port);
      return err;
    }
    ...
    if (_cfg.use_adc)
    {
      err = _mic_i2s_set_adc(_cfg.i2s_port, (gpio_num_t)_cfg.pin_data_in);
      if (err != ESP_OK) { _mic_i2s_driver_uninstall(_cfg.i2s_port); }
    }
#endif
  ...
  void Mic_Class::mic_task(void* args)
  {
    ...
    if (raw_clk_ok && ESP_OK == _mic_i2s_start(self->_cfg.i2s_port))
    {
    ...
    _mic_i2s_read(self->_cfg.i2s_port, src_buf, read_bytes, &src_len, 10);
    _mic_i2s_read(self->_cfg.i2s_port, src_buf, read_bytes, &src_len, 10);
      ...
      for (;;)
      {
        if (src_idx >= src_len)
        {
          ...
          for (;;)
          {
            ...
            if (ESP_OK == _mic_i2s_read(self->_cfg.i2s_port, src_buf, read_bytes, &src_len, 100)
...
// exists to verify, so only the verified classic ESP32 is handled here.
    ...
    _mic_i2s_stop(self->_cfg.i2s_port);
  ...
  bool Mic_Class::_begin_locked(void)
  {
    ...
    if (res)
    {
      ...
      { // Success means the port runs: wait for the task to enable the
        ...
        // this, an _mic_i2s_start() failure would leave record() returning
